...
class CommentBox extends React.Component {
    constructor(props) {
        ...
        this.state = {
            comments: [],
            comment_id : window.location.pathname.split("/").filter(filterNan).pop(),
            article : null,
            currentUser:null,
            recomment_origin_id : "", 
        };
    ...
    componentDidMount() {
        let getArticles = () => {
            axios.post('/comments/view/'+this.state.comment_id)
                .then(res => {
                    this.setState({
    ...
    render() {
        ...
        if(isAgree){
            if(this.state.currentUser!=null){
                new_input_comment = e("form",{method:"POST",action:"/comments/add/"+this.state.comment_id},
                    e("input",{type:"text",name:"comment",size:"50",placeholder:"댓글을 입력해주세요"}),
